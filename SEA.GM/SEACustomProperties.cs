			...
			try
			{
				...
				CustomProperty<SMI.IMyMotorStator, DeltaLimitSwitch<SMI.IMyMotorStator>>.ControlProperty("Virtual Angle",
						...
						(context) => context.Init(
								...
								(block) =>
								{
									var lowerLimit = block.GetMinimum<float>("Angle");
									var upperLimit = block.GetMaximum<float>("Angle");
									if (float.IsInfinity(lowerLimit) && float.IsInfinity(upperLimit))
										return MyMath.ShortestAngle(block.Angle, context.Limit);
									else
									{
										if (context.Limit < lowerLimit) context.Value = lowerLimit;
										else if (context.Limit > upperLimit) context.Value = upperLimit;
